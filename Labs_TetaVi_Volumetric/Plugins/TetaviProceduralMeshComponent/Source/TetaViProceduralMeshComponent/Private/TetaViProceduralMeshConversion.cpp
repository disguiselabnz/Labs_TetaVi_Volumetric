...
#include "TetaViProceduralMeshConversion.h"
...
TMap<UMaterialInterface*, FPolygonGroupID> BuildMaterialMap(UTetaViProceduralMeshComponent* ProcMeshComp, FMeshDescription& MeshDescription)
{
	...
	{
		FTetaViProcMeshSection *ProcSection =
...
void MeshDescriptionToProcMesh( const FMeshDescription& MeshDescription, UTetaViProceduralMeshComponent* ProcMeshComp )
{
	...
	for ( const FTriangleID TriangleID : MeshDescription.Triangles().GetElementIDs() )
	{
		...
		for ( const FVertexInstanceID& VertexInstanceID : MeshDescription.GetTriangleVertexInstances( TriangleID ) )
		{
			...
			else
			{
				...
				SectionData.Positions.Add( (FVector3f)VertexPositions[VertexID] );
				SectionData.Triangles.Add( NewVertexIndex );
				SectionData.Normals.Add( (FVector3f)Normals[VertexInstanceID] );
				SectionData.UV0.Add( FVector2f(UVs[VertexInstanceID]) );
				SectionData.Tangents.Add( FTetaViProcMeshTangent( (FVector3f)Tangents[VertexInstanceID], BinormalSigns[VertexInstanceID] < 0.f ) );
			}
		}
	}
...
FMeshDescription BuildMeshDescription( UTetaViProceduralMeshComponent* ProcMeshComp )
{
	...
	for (int32 SectionIdx = 0; SectionIdx < NumSections; SectionIdx++)
	{
		...
		{
			FTetaViProcMeshVertex &Vert = ProcSection->ProcVertexBuffer[VertexIndex];
		...
		for (int32 IndiceIndex = 0; IndiceIndex < NumIndices; IndiceIndex++)
		{
			...
			FTetaViProcMeshVertex &ProcVertex = ProcSection->ProcVertexBuffer[VertexIndex];
